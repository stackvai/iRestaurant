...
class OptionController extends Controller
{
    ...
    public function store(Request $request)
    {
        $validated = $request->validate([
            ...
            'option_items.*.is_default' => 'boolean|integer',
        ...
        foreach ($validated['option_items'] as $index => $item) {
        ...
        foreach ($validated['items'] as $itemOption) {
            $option->items()->attach($itemOption['item_id'], [
                ...
                'min_selection' => $itemOption['min_selection'] ?? 1,
                'max_selection' => $itemOption['max_selection'] ?? 1,
                'multiple_selection' => $itemOption['multiple_selection'] ?? false,
            ]);
        }

        return redirect()->route('options.index')->with('success', 'Option and items created successfully.');
    }
