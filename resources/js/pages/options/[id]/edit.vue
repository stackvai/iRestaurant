...
const { option, categories } = defineProps({
    option: Object,
    categories: Array,
});
console.log(option);
...
const form = useForm({
    ...
    min_selection: option.items[0].pivot.min_selection ?? 0,
    max_selection: option.items[0].pivot.max_selection ?? 1,
    multiple_selection: option.items[0].pivot.multiple_selection ?? false,

    option_items: option.option_items.map((item, index) => ({
        name: item.name,
        description: item.description,
        extra_price: item.extra_price,
        tax: item.tax ?? 0,
        image: item.image ?? null, // base64 string
        is_default: index === 0,
        position: index + 1,
    })),
    ...
    items: option.items.map((item) => ({
        item_id: item.id,
        is_required: item.pivot?.is_required ?? false,
        min_selection: item.pivot?.min_selection ?? 0,
        max_selection: item.pivot?.max_selection ?? 1,
        multiple_selection: item.pivot?.multiple_selection ?? false,
    })),
});
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="p-6">
            <div class="flex flex-row items-center justify-between">
                <h1 class="text-2xl font-bold">Edit Options</h1>
                <Link href="/options">
            ...
            <hr class="my-6" />
            <!-- Create Form -->
            <form @submit.prevent="updateOption" class="relative">
                <div class="flex flex-row gap-4">
                    <div class="w-[55%] rounded-xl p-2 shadow">
                        ...
                        <div class="p-2">
                            ...
                            <div class="mb-3 flex flex-row justify-between">
                                <Checkbox :model-value="form.is_active === 0? false :true " @update:model-value="form.is_active = $event" label="Active"  />
                                <Checkbox
                                    :model-value="form.multiple_selection === 0? false :true"
                        ...
                        <div class="">
                            ...
                            >
                                <div class="flex flex-row items-center justify-start gap-2">
                                    <div class="w-[45%]">
                                        <ImageUploader v-model="item.image" type="square" :existingUrl="item.image" />
                                    </div>
                    ...
                    <div class="w-[45%] rounded-xl p-4 shadow">
                        <div class="flex flex-row items-center justify-between">
                            ...
                            <Button type="submit" class="rounded px-4 py-2" :disabled="form.processing">
                                <Icon name="hardDriveDownload" /> Update
                            </Button>
                        </div>
                        <hr class="my-3" />
                        <div>
