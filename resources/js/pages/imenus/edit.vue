            ...
            <div class="mx-auto h-auto max-w-5xl overflow-clip">
                <form @submit.prevent="submit" class="flex h-full items-start justify-center gap-4">
                    <div class="flex h-full w-md flex-col items-start justify-between gap-2 p-4">
                        ...
                        <div class="w-full">
                            <ImageUploader v-model="form.image" :existingUrl="form.image" type="video" label="Image Upload" />
                        </div>
                    ...
                    <div class="p-4">
                        ...
                        <div v-for="(slots, day) in form.availability" :key="day" class="space-y-1">
                            ...
                            <div v-for="(slot, index) in slots" :key="index" class="flex items-center gap-2">
                                ...
                                <Button size="sm" variant="outline" type="button" @click="form.availability[day].splice(index, 1)">
                                    <MyIcon name="trash" />
                                </Button>
                            </div>
                        </div>
...
const breadcrumbs = [
    ...
    { title: 'Edit', href: `/imenus/${props.menu.id}/edit` },
];
...
const form = useForm({
    ...
    branch_id: props.menu.branch_id,
    availability: props.menu.availability || {
        Monday: [{ from: '', to: '' }],
        Tuesday: [{ from: '', to: '' }],
        Wednesday: [{ from: '', to: '' }],
        Thursday: [{ from: '', to: '' }],
...
const options = computed(() => [
    {
        name: 'Branches',
        children: props.branches.map((branch) => ({ id: branch.id, name: branch.name })),
    },
]);
</script>
